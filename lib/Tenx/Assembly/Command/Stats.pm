...
my $usage = "$0 <SCAFFOLDS/CONTIGS.FASTA> <?MIN_GAP_SIZE? default=1>";
...
if( $ARGV[1] ) {
    if( not $ARGV[1] =~ /^\d+$/ ) {
	die "Expected MIN_GAP_SIZE to be a number, not: $ARGV[1]\n";
    }
    $min_Ns_for_gap = $ARGV[1];
}
...
while( my $seq = $reader->next_seq ) {
   #print 'Processing '.$seq->id."\n";
    ...
    set_breakdown_length( $scaffold_length, 'SCAF' );
   ...
   #print Dumper \@contigs;
	...
	set_breakdown_length( $contig_length, 'CTG' );
...
#>>>>>> Scaffolds > 1M: 0 ( total: 0 bases )
#>>>>>> Scaffold 250K--1M: 52 (total_ 10,000,000 bases)
#>>>>>> Scaffold 100K--250K: 37 ( etc .. )
#>>>>>> Scaffold 10--100K: 405
#>>>>>> Scaffold 5--10K: 80
#>>>>>> Scaffold 2--5K: 13
#>>>>>> Scaffold 0--2K: 2
